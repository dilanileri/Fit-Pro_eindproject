                    ...
                    {workout.exercises.length === 0 ? (
                        ...
                        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                            {workout.exercises.map((exercise) => (
                                <Link
                                    key={exercise.id}
                                    href={`/member/exercises/${exercise.id}?sets=${exercise.pivot?.sets ?? ''}&reps=${exercise.pivot?.reps ?? ''}&rest=${exercise.pivot?.rest_seconds ?? ''}`}
                                    className="group flex min-h-[260px] flex-col justify-between rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-5 shadow-lg shadow-black/20 transition hover:-translate-y-1 hover:border-sky-400/40 md:p-6"
                                >
                                    <div>
                                        {exercise.image && (
                                            <div className="mb-5 overflow-hidden rounded-3xl border border-slate-800">
                                                <img
                                                    src={exercise.image}
                                                    alt={exercise.name}
                                        ...
                                        <div>
                                            <div className="mb-5 flex items-start justify-between gap-4">
                                                ...
                                                {exercise.muscle_group && (
                                                    <span className="rounded-full border border-sky-700 bg-black/25 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-sky-400">
                                                        {exercise.muscle_group}
                                                    </span>
                                                )}
                                            </div>
                                        ...
                                        <div className="mt-6">
                                            ...
                                            <div className="mt-5 flex items-center justify-between border-t border-slate-800 pt-5">
                                                ...
                                                <ArrowRight className="h-5 w-5 text-slate-600 transition group-hover:translate-x-1 group-hover:text-sky-400" />
                                            </div>
                                        </div>
                                    </div>
                                </Link>
